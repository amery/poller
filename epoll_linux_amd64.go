...
//    }
//
// and then epollwait will return us *event's that contain the pointer to the *WaitPollable that
...
//    the bottom 4 bytes are preserved, so calling a method on the *WaitPollable returned will
//    segfault.
//
...
// unsafe to convert the value to/from a *WaitPollable as required. For 32 bit platforms, the
...
func (e *event) setdata(p *WaitPollable) {
	*(**WaitPollable)(unsafe.Pointer(&e.data[0])) = p
}

func (e *event) getdata() *WaitPollable {
	return *(**WaitPollable)(unsafe.Pointer(&e.data[0]))
}
